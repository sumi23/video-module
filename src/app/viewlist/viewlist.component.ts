...
videoobj:Video;
  listVideoById(){
    this.obj.listVideoById(this.id).subscribe((result:any)=>
    {
      ...
      this.videoobj=result.data;
      console.log("video object is"+JSON.stringify(this.video));
    }
    );
  }
 check()
 {
  return this.videoobj.videoContent.some(item => item.type === 'referenceartifact' );
 }
 checkSamProg(){
   for(let i=0;i<this.videoobj.videoContent.length;i++)
   {
     if(this.videoobj.videoContent[i].type=='sampleprogram')
       return this.videoobj.videoContent[i];
   }
 }
  fileName:string;
  downloadFile(file) {
